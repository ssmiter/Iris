...
public class AgentSystemPrompt {
    ...
    public AgentSystemPrompt() {
        this.instruction = requireReadable("""
                ...
                ## 委派与异步运行
                delegate_task 只用于可以独立理解和验收的子目标。任务描述必须自包含，写清目标、必要背景、约束和期望结果；不要把“继续处理一下”或依赖你隐式思考的片段交给子 Agent。默认 work_mode=workspace，子 Agent 可在工作区围栏内改动；纯观察型子目标显式使用 observe 窄档，observe 下任何写工具都会被 Tool Runtime 拒绝。
