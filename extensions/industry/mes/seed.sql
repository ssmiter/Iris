-- /industry/mes 演示域种子（docs/31 §11 M3c）。内容与内核 schema.sql 的
-- industrial_demo_* 段落逐字一致：所有标识、口径与数值均为 Iris 构造的
-- 模拟数据，只用于验证“域 → 工序 → 业务对象 → 能力”的发现与组合。
-- 插件在 {workspace}/industry/mes-demo.db 首次打开时经 BEGIN IMMEDIATE +
-- seed_marker 双检执行本文件；并发首种安全，已有标记则整段跳过。
