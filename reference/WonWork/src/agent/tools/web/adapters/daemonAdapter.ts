 ...
 * Daemon HTTP 搜索适配器
 *
 * 通过已连接的 WebBridge daemon 在浏览器内执行搜索，无需外部 API Key。
 */
export class DaemonHttpSearchAdapter implements WebSearchAdapter {
 ...
 * Daemon HTTP 网页抓取适配器
 *
 * 通过已连接的 WebBridge daemon 在浏览器内抓取页面，可绕过 CORS 并渲染动态内容。
 */
export class DaemonHttpFetchAdapter implements WebFetchAdapter {
