...
import { Globe, Loader2, CheckCircle2, XCircle, ChevronDown, ChevronRight } from 'lucide-react'
import { cn } from '@/utils'
import type { ToolThinkingRendererProps } from '@/agent/tools/toolRenderRegistry'

export function WebFetchThinking({
  status,
  ...
  const statusConfig = {
    planning: {
      ...
      icon: <Globe size={14} className="text-surface-400" />,
      badgeClass: 'bg-surface-100 text-surface-500',
    },
    coding: {
      label: t('chat.thinkingProcess.running'),
    ...
    completed: {
      label: t('chat.thinkingProcess.completed'),
      icon: <CheckCircle2 size={14} className="text-green-500" />,
      badgeClass: 'bg-green-50 text-green-600',
    },
    error: {
      label: t('chat.thinkingProcess.executionError'),
